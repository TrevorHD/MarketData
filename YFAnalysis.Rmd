...
* **Volume**: average number of daily shares traded (billions)
...
testlist <- lapply(names(sort(test, decreasing = TRUE)[1:10]), FUN = glm, data = SPWeeklyTrain, family = binomial)

lapply(testlist, FUN = coef) %>% 
  lapply(FUN = as.list) -> coeflist

# Create table of coefficients, AIC, max VIF, and accuracy
meh <- cbind(do.call(rbind.fill, lapply(coeflist, as.data.frame)),
             rep(0, 10),
             sapply(testlist, FUN = AIC),
             as.numeric(sort(test, decreasing = TRUE)[1:10]))
names(meh)[c(1, 9, 10, 11)] <- c("Intercept", "Lag1", "AIC", "Accuracy")

# max vif
sapply(sapply(testlist, FUN = vif), FUN = max)

# Reorder columns
meh <- meh[, c(1, 9, 2:4, 8, 5:7, 10:11)]
...
# Create table of coefficients, AIC, max VIF, and accuracy
meh2 <- cbind(do.call(rbind.fill, lapply(coeflist, as.data.frame)),
             sapply(testlist, FUN = AIC),
             as.numeric(sort(test, decreasing = TRUE)[1:10]))
names(meh2)[c(1, 10, 11)] <- c("Intercept", "AIC", "PPV")

# max vif
sapply(sapply(testlist, FUN = vif), FUN = max)

# Reorder columns
meh2 <- meh2[, c(1, 8, 2:3, 6:7, 9, 4, 5, 10:11)]
