...
We can also examine the training accuracy of models that include two-way interactions; the total number of two-way interaction terms plus single terms is 36, so thus the total number of interactive models that we can form is \begin{equation} \sum_{i=1}^{36}\frac{36!}{i!(36-i)!} \end{equation} which is equal to 68719476735.
...
logreg.lcv <- function(i, dataTrain, output){
  predVec[-i] %>% 
    paste0(., collapse = "+") %>% 
    paste0("Direction~", .) %>% 
    logreg(dataTrain = SPWeeklyTrain, output = "accuracy") %>% 
    return()}

# Function to return accuracy and new list of predictors after removing a predictor
logreg.bwd <- function(predVec){
  accs <- sapply(1:length(predVec), FUN = logreg.lcv, dataTrain = SPWeeklyTrain, output = "accuracy")
...
logreg.fwd <- function(predVecRemain, predVecFwd){
  accs <- sapply(1:length(predVecRemain), FUN = logreg.lcv2, dataTrain = SPWeeklyTrain, output = "accuracy")
...
# Apply backward step algorithm
combn(names(SPWeeklyTrain)[-c(2:3)], 2) %>%
    apply(FUN = paste0, MARGIN = 2, collapse = ":") %>%
    c(names(SPWeeklyTrain)[-c(2:3)]) -> predVec
prevmaxacc <- c(0.5, 0.5)
predVec <- logreg.bwd(predVec)$predVec
maxacc <- logreg.bwd(predVec)$maxacc
prevmaxacc <- c(prevmaxacc, maxacc)
while(maxacc >= prevmaxacc[length(prevmaxacc) - 1]){
  predVec <- logreg.bwd(predVec)$predVec
  maxacc <- logreg.bwd(predVec)$maxacc
  prevmaxacc <- c(prevmaxacc, maxacc)
  if(length(predVec) == 1){
    break}}
prevmaxacc[length(prevmaxacc) - 1]

# Apply foward step algorithm
combn(names(SPWeeklyTrain)[-c(2:3)], 2) %>%
    apply(FUN = paste0, MARGIN = 2, collapse = ":") %>%
    c(names(SPWeeklyTrain)[-c(2:3)]) -> predVecRemain
prevmaxacc <- c(0.5, 0.5)
predVecFwd <- c()
maxacc <- logreg.fwd(predVecRemain, predVecFwd)$maxacc
predVecFwd <- logreg.fwd(predVecRemain, predVecFwd)$predVecFwd
predVecRemain <- logreg.fwd(predVecRemain, predVecFwd)$predVecRemain
prevmaxacc <- c(prevmaxacc, maxacc)
while(maxacc >= prevmaxacc[length(prevmaxacc) - 1]){
  predVecFwd <- logreg.fwd(predVecRemain, predVecFwd)$predVecFwd
  predVecRemain <- logreg.fwd(predVecRemain, predVecFwd)$predVecRemain
