...
returns <- function(formula, data, final = FALSE, lrisk = FALSE){
  if(formula != "none"){
    glm.fit <- glm(formula = formula, data = data, family = "binomial")
    glm.probs <- predict(glm.fit, SPWeekly, type = "response")
    glm.pred <- rep("Down", length(glm.probs))
    glm.pred[glm.probs >= 0.5] <- "Up"
    pcts <- data$PctChange
    if(lrisk == FALSE){
...
for(i in 1:6){
  print(returns(SPWeeklyLMod[[i]], data = SPWeekly, final = TRUE))}
...
for(i in c(1, 3, 4)){
  ...
  text(x = c(0.736, 1, 0.8)[j], y = rep(3, 3)[j], labels = c("Model 1", "Model 3", "Model 4")[j], adj = 1)
...
for(i in c(2, 5, 6)){
  ...
  lines(lowess(predict(SPWeeklyLModList[[i]]), residuals(SPWeeklyLModList[[i]])), col = "red", lwd = 1.5)
  abline(h = c(-2, 2), col = "red", lty = 3, lwd = 1.5)
  text(x = c(0.8, 1, 1)[j], y = rep(3, 3)[j], labels = c("Model 2", "Model 5", "Model 6")[j], adj = 1)
...
``` {r Figure21, fig.height = 7.8, fig.cap = "\\label{fig:Figure21} Relative value of the S\\&P 500 over time compared to its 1986 value (left) and percent change for weeks in which the model did not predict an increase (right) for logistic regression models selected using accuracy. The black line represents a buy-and-hold strategy, while the blue line represents a strategy of selling all assets before a predicted decrease and then purchasing them back the next time there is a predicted increase. The purple line represents a less risky strategy of only selling 35\\% of assets before a predicted decrease and then purchasing them back the next time there is a predicted increase."}
...
for(i in c(1, 3, 4)){
  ...
  # Plot returns
  plot(returns(SPWeeklyLMod[[i]], data = SPWeekly), type = "l", col = "blue", xaxt = "n",
       xlab = "Year", ylab = "Proportion of Initial Value", ylim = c(0, 40))
  axis(1, at = nrow(SPWeekly)*(0:7)/7, labels = seq(1986, 2021, length.out = 8), cex.axis = 0.85)
  lines(returns(SPWeeklyLMod[[i]], data = SPWeekly, lrisk = TRUE), col = "purple")
  lines(returns("none", data = SPWeekly))
  text(x = rep(20, 3)[j], y = rep(39, 3)[j], labels = c("Model 1", "Model 3", "Model 4")[j], adj = 0)
  
  # Plot increases/decreases on weeks when mode predicts a decrease
  glm.fit <- glm(SPWeeklyLMod[[i]], data = SPWeekly, family = "binomial")
  glm.probs <- predict(glm.fit, SPWeekly, type = "response")
  glm.pred <- rep("Down", length(glm.probs))
  glm.pred[glm.probs >= 0.5] <- "Up"
  plotdat <- data.frame(glm.pred, SPWeekly$Direction, SPWeekly$PctChange)
  plotdat <- subset(plotdat, glm.pred == "Down")
  plot(as.numeric(rownames(plotdat)), plotdat$SPWeekly.PctChange, pch = 16, xaxt = "n",
       xlim = c(0, nrow(SPWeekly)), col = ifelse(plotdat$SPWeekly.PctChange < 0, "red", "green"),
       xlab = "Year", ylab = "Weekly Percent Change", ylim = c(-16, 16))
  axis(1, at = nrow(SPWeekly)*(0:7)/7, labels = seq(1986, 2021, length.out = 8), cex.axis = 0.85)
  abline(h = mean(plotdat$SPWeekly.PctChange[plotdat$SPWeekly.PctChange < 0]), lty = 2, col = "red")
  abline(h = mean(plotdat$SPWeekly.PctChange[plotdat$SPWeekly.PctChange > 0]), lty = 2, col = "green")
  text(x = rep(20, 3)[j], y = rep(15, 3)[j], labels = c("Model 1", "Model 3", "Model 4")[j], adj = 0)
  text(x = rep(400, 3)[j], y = rep(15, 3)[j], labels = as.numeric(table(plotdat$SPWeekly.Direction)[1]),
       col = "red", adj = 0)
  text(x = c(510, 550, 510)[j], y = rep(15, 3)[j], labels = "/", adj = 0)
  text(x = c(540, 580, 540)[j], y = rep(15, 3)[j], labels = as.numeric(table(plotdat$SPWeekly.Direction)[2]),
       col = "green", adj = 0)}
...
``` {r Figure22, fig.height = 7.8, fig.cap = "\\label{fig:Figure22} Relative value of the S\\&P 500 over time compared to its 1986 value (left) and percent change for weeks in which the model did not predict an increase (right) for logistic regression models selected using PPV The black line represents a buy-and-hold strategy, while the blue line represents a strategy of selling all assets before a predicted decrease and then purchasing them back the next time there is a predicted increase. The purple line represents a less risky strategy of only selling 35\\% of assets before a predicted decrease and then purchasing them back the next time there is a predicted increase."}
...
for(i in c(2, 5, 6)){
  ...
  abline(h = mean(plotdat$SPWeekly.PctChange[plotdat$SPWeekly.PctChange < 0]), lty = 2, col = "red")
  abline(h = mean(plotdat$SPWeekly.PctChange[plotdat$SPWeekly.PctChange > 0]), lty = 2, col = "green")
  text(x = rep(20, 3)[j], y = rep(15, 3)[j], labels = c("Model 2", "Model 5", "Model 6")[j], adj = 0)
  text(x = rep(400, 3)[j], y = rep(15, 3)[j], labels = as.numeric(table(plotdat$SPWeekly.Direction)[1]),
       col = "red", adj = 0)
  text(x = c(510, 550, 510)[j], y = rep(15, 3)[j], labels = "/", adj = 0)
  text(x = c(540, 580, 540)[j], y = rep(15, 3)[j], labels = as.numeric(table(plotdat$SPWeekly.Direction)[2]),
