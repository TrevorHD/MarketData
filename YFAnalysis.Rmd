...
Doing so now results in a model with a training accuracy of 57.08% and 7 terms; while the training accuracy is lower, there are fewer terms than our model resulting from backward selection. We can also use the same forward and backward algorithms but select models using PPV instead of accuracy. If we do so using the backward step algorithm, we get a model with a PPV of 59.24% and 32 terms; using the forward step algorithm, we get a model with a PPV of 57.68% and 10 terms.
...
for(i in 1:6){
  print(returns(SPWeeklyLMod[[i]], data = SPWeeklyTest, final = TRUE))}
...
for(i in c(1, 3, 4)){
  ...
  text(x = c(0.9, 1, 0.8)[j], y = rep(3, 3)[j], labels = c("Model 1", "Model 3", "Model 4")[j], adj = 1)
...
for(i in c(1, 3, 4)){
  ...
  # Plot returns
  plot(returns(SPWeeklyLMod[[i]], data = SPWeeklyTest), type = "l", col = "blue", xaxt = "n",
  ...
  lines(returns(SPWeeklyLMod[[i]], data = SPWeeklyTest, lrisk = TRUE), col = "purple")
  lines(returns("none", data = SPWeeklyTest))
  text(x = rep(10, 3)[j], y = rep(7.8, 3)[j], labels = c("Model 1", "Model 3", "Model 4")[j], adj = 0)
  ...
  text(x = rep(10, 3)[j], y = rep(15, 3)[j], labels = c("Model 1", "Model 3", "Model 4")[j], adj = 0)
...
``` {r Figure22, fig.height = 7.8, warning = FALSE, fig.cap = "\\label{fig:Figure22} Relative value of the S\\&P 500 from 2011-2021 (left) and percent change for weeks in 2011-2021 for which an increase was not predicted (right) based on models fit to the 1986-2010 data; the three models here were selected using PPV. The black line represents a buy-and-hold strategy, while the blue line represents a strategy of selling all assets before a predicted decrease and then purchasing them back the next time there is a predicted increase. The purple line represents a less risky strategy of only selling 35\\% of assets before a predicted decrease and then purchasing them back the next time there is a predicted increase."}
...
for(i in c(2, 5, 6)){
  ...
  lines(returns(SPWeeklyLMod[[i]], data = SPWeeklyTest, lrisk = TRUE), col = "purple")
  lines(returns("none", data = SPWeeklyTest))
  text(x = rep(10, 3)[j], y = rep(7.8, 3)[j], labels = c("Model 2", "Model 5", "Model 6")[j], adj = 0)
  ...
  abline(h = mean(plotdat$SPWeeklyTest.PctChange[plotdat$SPWeeklyTest.PctChange < 0]), lty = 2, col = "red")
  abline(h = mean(plotdat$SPWeeklyTest.PctChange[plotdat$SPWeeklyTest.PctChange > 0]), lty = 2, col = "green")
  text(x = rep(10, 3)[j], y = rep(15, 3)[j], labels = c("Model 2", "Model 5", "Model 6")[j], adj = 0)
  text(x = rep(110, 3)[j], y = rep(15, 3)[j], labels = as.numeric(table(plotdat$SPWeeklyTest.Direction)[1]),
       col = "red", adj = 0)
  text(x = rep(140, 3)[j], y = rep(15, 3)[j], labels = "/", adj = 0)
  text(x = rep(150, 3)[j], y = rep(15, 3)[j], labels = as.numeric(table(plotdat$SPWeeklyTest.Direction)[2]),
