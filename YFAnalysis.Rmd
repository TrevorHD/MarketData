...
# Function to fit logistic regression to all possible combinations of n variables AND interactions
# Outputs value and combination of variables for desired prediction metric
# This is way too large for n = 36; need to fix
logreg.combInt <- function(dataTrain, output, n){
  combn(names(dataTrain)[-c(2:3)], 2) %>%
    apply(FUN = paste0, MARGIN = 2, collapse = ":") %>%
    c(names(dataTrain)[-c(2:3)]) %>%
    combn(n) %>% 
      apply(FUN = paste0, MARGIN = 2, collapse = "+") %>%
      paste0("Direction~", .) %>% 
      sapply(FUN = logreg, dataTrain = dataTrain, output = output) %>% 
...
SPWeeklyTopInt <- unlist(sapply(1:36, FUN = logreg.combInt, dataTrain = SPWeeklyTrain, output = "accuracy"))
sort(SPWeeklyTopInt, decreasing = TRUE)[1:10]
